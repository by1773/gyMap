  ...
  <Card>
    <simple-table :columns="tableMeta.columns"
      ...
      <Button type="error"
              ...
              v-hasPermission="$access.membership.login_attempts.clear"
      ...
      <Button type="primary"
              ...
              icon="md-download"
              class="attempts-toolbar"
              v-hasPermission="$access.membership.login_attempts.export"
...
import { exportLoginAttempts, clearLoginAttempts, pageLoginAttempts } from '@/api/membership/login-attempts'
export default {
  name: 'LoginAttempts',
  components: {
    SimpleTable
    ...
    return {
      tableMeta: {
        columns: [
          ...
          { title: '登录结果', key: 'result', minWidth: 160 },
  ...
  methods: {
    ...
    onCleanAll () {
      this.$Modal.confirm({
        ...
        onOk: () => {
          clearLoginAttempts().then(result => {
    ...
    onExportExcel () {
      ...
      exportLoginAttempts(input).then(({ data, headers }) => {
    ...
    loadData (page, keyword) {
      ...
      pageLoginAttempts(input).then(({ data }) => {
